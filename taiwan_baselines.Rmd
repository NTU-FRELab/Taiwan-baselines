...
knitr::purl("taiwan_baselines.Rmd", documentation = 0)
```
...
**Note**: All figures were produced in R (version 4.2.0). Final figure aesthetics and panel arrangements were adjusted in Inkscape (version 1.3.2) for publication formatting purposes. All data representations remain unaltered.
...
2. Import factors (Region, MPA protection level, Morpho-functional group)
```{r}
...
### Figure 1. Light and temperature conditions of seawater surrounding Taiwan

**Note**: The code below generates Figure 1. For panel C, the code lines used to plot Marine Protected Area (MPA) spatial layers are provided as annotations. These layers must be requested directly from the responsible Taiwanese governmental agencies, such as the Ocean Conservation Administration (OCA; https://www.oca.gov.tw/en/) or the Ocean Affairs Council (OAC; https://www.oac.gov.tw/en/).

1. Import environmental data and Taiwan landmass
```{r}
...
A. Average surface PAR (x10^6 umol m-2 d-1) derived from VIIRSN on the Suomi-NPP, NASA (resolution: 4 km, period: 01/15/2012-01/15/2022) with study sites in red
```{r}
PAR <- ggplot() +
...
B. Minimum monthly sea surface temperature (SST, °C) from NOAA Global Coral Bleaching Monitoring (resolution: 5 km; period: 01/16/1986-12/16/2022) with study regions in blue
```{r}
MinSST <- ggplot() +
...
C. Maximum monthly sea surface temperature (SST, °C) from NOAA Global Coral Bleaching Monitoring (resolution: 5 km; period: 01/16/1986-12/16/2022) *without MPA delineation* 
```{r}
...
3. Combine all 3 environmental maps into a single figure
```{r fig.width=18, fig.height=7}
# Arrange environmental maps together
ggpubr::ggarrange(PAR2, MinSST2, MaxSST2, ncol=3, nrow = 1)
```

### Benthic composition: regional differences and environmental effects

#### Regional benthic composition visualization

1. Import "Data summary - Regional Average Benthic Major Categories" and filter categories contributing at least 2.0% in at least one region (9 of 13 biotic and abiotic major categories)
```{r}
### Benthic patterns ###
# Regional summary
...
2. Calculate national average and pivot table to long format
```{r}
# National_average
...
3. Generate and arrange regional circular plots
...
1. Distance matrix, betadisper, permutest, tukey, PERMANOVA and boxplot (Figure 3A and B)
```{r}
### Regional differences ###
# Distance matrix
...
3. Dufrêne and Legendre indicator species analyses
```{r}
...
#### Environmental influence on benthic compositions

GAMs (Supplementary figure 2)
```{r}
### GAMs ###
...
### Benthic differences between protected and unprotected areas

#### Comparing selected major categories (Scleractinian coral, CCA and Turf) cover between locations under different protection levels (Figure 4)
1. Express targeted major categories into percentage
```{r}
...
3. Permutation based Kruskal-Wallis and pairwise Wilcoxon permutation test
